...
| `chat.dm.messages`   | `receiver_id` | All instances   | Direct messages                      |
...
```json
{
  "type":        "message | dm | typing | presence",
  "room_id":     "<uuid>",
  "sender_id":   "<uuid>",
  "to_user_id":  "<uuid>",
...
| `chat.dm.messages`   | `hub.BroadcastUser(receiverID)` + `hub.BroadcastUser(senderID)` — echo to both sides |
